...
def Network(self, x, is_training=tf.bool):
    ...
    num = 1
    print_fn(x)
    print_fn('=' * 10 + 'ResNet_softmax' + '=' * 10)
    kernel_regularizer = tf.contrib.layers.l2_regularizer(
        scale=self.FLAGS.weight_decay)
    #------------------------------------------------------------
    x = tf.reshape(x, [-1, self.data_shape, 1], namebase+'reshape')
    shortcut = tf.layers.conv1d(x, 16*frac, 1, 4, padding='same',
                               activation=None,
    ...
    with slim.arg_scope(arg_scope(weight_decay=self.FLAGS.weight_decay)):
        ...
        net = tf.layers.conv1d(net, 16*frac, 3, padding='same',
                              activation=None, 
                              use_bias=False,
                              kernel_regularizer=kernel_regularizer,
...
#----------------------------------------
    indicates = tf.one_hot(y_, depth=6)
