...
def Network(self, x, is_training=tf.bool):
    ...
    with slim.arg_scope(arg_scope(weight_decay=self.FLAGS.weight_decay)):
        ...
        net = layer(x, 8*frac, 3, name=namebase+'conv%d'%num, padding='same')
        net = tf.layers.max_pooling1d(net, 3, 2,
                                      padding='same',
                                      name=namebase+'maxpool%d'%num)
        num += 1
...
#
        ...
        num += 1
#
        net = tf.layers.conv1d(net, 32*frac, 3, padding='same',
                              activation=None, 
                              kernel_regularizer=kernel_regularizer,
...
#----------------------------------------
#   indicates = tf.one_hot(indicates, depth=2)
